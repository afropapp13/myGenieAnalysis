#ifndef GenieAnalysis_h
#define GenieAnalysis_h
...
double XMin = 3, XMax = 253;
double YMin = -110, YMax = 110;
double ZMin = 5, ZMax = 1032;
...
TString FileName = "argon40_CCinclMEC_BNBFlux";

TString Extension = "";
//TString Extension = "default";
//TString Extension = "hA2015";

TString FullName = Path + FileName + Extension;

double XaxisOffset = 128.2; // cm
double YaxisOffset = 0.; // cm
double ZaxisOffset = 518.4; // cm
...
public :
   ...
   virtual bool     InFV(TVector3 VertexLocation);
   virtual void     SetLArTools();
   virtual double   Get_protonMomentumFromRange( double range );
   virtual double   Get_muonMomentumFromRange( double range );
   virtual double   Get_protonRangeFromMomentum( double momentum );
   virtual double   Get_muonRangeFromMomentum( double momentum );
};
...
GenieAnalysis::~GenieAnalysis()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t GenieAnalysis::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t GenieAnalysis::LoadTree(Long64_t entry)
{
...
void GenieAnalysis::Init(TTree *tree)
{
...
void GenieAnalysis::Show(Long64_t entry)
{
...
Int_t GenieAnalysis::Cut(Long64_t entry)
{
...
bool GenieAnalysis::InFV(TVector3 VertexLocation) {

	bool IsContained = true;

	if ( VertexLocation.X() < XMin || VertexLocation.X() > XMax) { IsContained = false; }
	if ( VertexLocation.Y() < YMin || VertexLocation.Y() > YMax) { IsContained = false; }
	if ( VertexLocation.Z() < ZMin || VertexLocation.Z() > ZMax) { IsContained = false; }
...
void GenieAnalysis::SetLArTools() {
	
	double E;

	const int NProtonBins = 31;
	
	// Proton range in gr/cm2
	Float_t Range_grampercm_proton[NProtonBins] =
	...
	Float_t KE_MeV_proton[NProtonBins] =
	...
	Float_t Range_grampercm_proton_LAr[NProtonBins];
	Float_t p_MeVc_proton[NProtonBins];

	for (int i = 0 ; i < NProtonBins; i++) {

	    Range_grampercm_proton_LAr[i] = Range_grampercm_proton[i] / LAr_density; // range [cm] = range [gr/cm^2] / rho [gr/cm^3]
	    E = ProtonMass + KE_MeV_proton[i];
	    p_MeVc_proton[i] = sqrt(E*E - ProtonMass*ProtonMass);
	}
	
	KE_vs_range_proton = new TGraph(NProtonBins, Range_grampercm_proton_LAr, KE_MeV_proton);
	KE_vs_range_proton_s3 = new TSpline3("KE_vs_range_proton", KE_vs_range_proton);
	p_vs_range_proton = new TGraph(NProtonBins, Range_grampercm_proton_LAr, p_MeVc_proton);
	p_vs_range_proton_s3 = new TSpline3("p_vs_range_proton", p_vs_range_proton);

	range_vs_p_proton = new TGraph(NProtonBins, p_MeVc_proton, Range_grampercm_proton_LAr);
	range_vs_p_proton_s3 = new TSpline3("range_vs_p_proton", range_vs_p_proton);

	// ----------------------------------------------------------------------------------------------------------------

	const int NMuonBins = 29;

	// Muon range in gr/cm2
	Float_t Range_grampercm_muon[NMuonBins] = {9.833E-1, 1.786E0, 3.321E0, 6.598E0, 1.058E1, 3.084E1, 4.250E1,
	...
	Float_t KE_MeV_muon[NMuonBins] = {10, 14, 20, 30, 40, 80, 100, 140, 200, 300, 400, 800, 1000,
	    1400, 2000, 3000, 4000, 8000, 10000, 14000, 20000, 30000,
	    40000, 80000, 100000, 140000, 200000, 300000, 400000
	};
	
	Float_t Range_grampercm_muon_LAr[NMuonBins];
	Float_t p_MeVc_muon[NMuonBins];
	for (int i = 0 ; i < NMuonBins; i++) {
	    Range_grampercm_muon_LAr[i] = Range_grampercm_muon[i] / LAr_density;
	    E = MuonMass + KE_MeV_muon[i];
	    p_MeVc_muon[i] = sqrt(E*E - MuonMass*MuonMass);
	}
...
double GenieAnalysis::Get_protonMomentumFromRange( double range ){  // return the result in MeV
	return p_vs_range_proton_s3 -> Eval(range);
}
double GenieAnalysis::Get_muonMomentumFromRange( double range ){	// return the result in MeV
	return p_vs_range_muon_s3 -> Eval(range);
}
double GenieAnalysis::Get_protonRangeFromMomentum( double momentum ){   // input the result in MeV, return in cm
	return range_vs_p_proton_s3 -> Eval(momentum);
}
double GenieAnalysis::Get_muonRangeFromMomentum( double momentum ){ // input the result in MeV, return in cm
